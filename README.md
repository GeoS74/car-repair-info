...
Маршрут:
```
GET /api/informator/cars/search/car
```

Параметры:
- search (строка запроса для поиска автомобиля по VIN-номеру, гос. номеру или модели)
- last (id последнего полученного объекта)
- limit (кол-во объектов в ответе)
...
Маршрут:
```
GET /api/informator/cars/:id
```
где `:id` - идентификатор автомобиля

##### Создание автомобиля

Маршрут:
```
POST /api/informator/cars
```
